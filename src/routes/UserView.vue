    ...
    <div class="dashboard-card glass">
      ...
      <main class="content-area">
        <Transition name="fade" mode="out-in">
            ...
            <div class="history-section">
              ...
              <ul v-if="searchHistory.length" class="history-list">
                <li v-for="item in searchHistory" :key="item.id">
                  ...
                  <small>{{ new Date(item.searched_at).toLocaleDateString() }}</small>
                </li>
              </ul>
          ...
          <div v-else-if="activeTab === 'profile'" key="profile" class="tab-content">
            <header class="tab-header">
          ...
          <div v-else-if="activeTab === 'security'" key="security" class="tab-content">
            <header class="tab-header">
              <h2>Segurança</h2>
            ...
            <div class="settings-form">
              <div class="form-group">
                ...
                <input v-model="security.email" type="email" placeholder="novo@email.com" />
                <button @click="updateEmail" class="btn-ghost" :disabled="loading">Atualizar Email</button>
              </div>
              ...
              <div class="form-group">
                ...
                <input v-model="security.password" type="password" placeholder="Mínimo 6 caracteres" />
                <button @click="updatePassword" class="btn-ghost" :disabled="loading">Atualizar Senha</button>
              </div>
            </div>
          </div>

          <div v-else-if="activeTab === 'themes'" key="themes" class="tab-content">
            <header class="tab-header">
            ...
            <div class="theme-section">
              ...
              <div class="theme-grid">
                ...
                >
                  ...
                  <Icon v-if="currentColor === t.id" icon="lucide:check" width="20" height="20" />
                </button>
              </div>
            </div>
          </div>
    ...
    <Transition name="fade">
      <div v-if="showLogoutModal" class="modal-overlay" @click="closeLogoutModal">
        <div class="modal glass" @click.stop>
          ...
          <p>Você tem certeza que deseja sair? Será necessário fazer login novamente para acessar seus conteúdos no AURA.</p>
          <div class="modal-actions">
            <button class="btn-cancel" @click="closeLogoutModal">Cancelar</button>
            <button class="btn-danger" @click="confirmLogout">Sair agora</button>
          </div>
        </div>
      </div>
    </Transition>
...
import { toast } from "vue-sonner"; // Se instalou o vue-sonner, senão use alert()
...
const availableColors = [
  { id: 'blue', label: 'Azul', preview: '#2ea4ff' },
  { id: 'red', label: 'Vermelho', preview: '#ff4757' },
  { id: 'green', label: 'Verde', preview: '#27ae60' },
  { id: 'purple', label: 'Roxo', preview: '#9b59b6' },
  { id: 'bw', label: 'P&B', preview: '#333333' },
];
...
// Correção do Nome: Tenta pegar do banco, se falhar, pega do metadata, se falhar, pega 'Visitante'
const firstName = computed(() => {
  const name = profile.value.full_name || user.value?.user_metadata?.full_name || "Visitante";
  return name.split(" ")[0];
});

const initials = computed(() => {
  const name = profile.value.full_name || user.value?.user_metadata?.full_name || "US";
  return name
    .split(" ")
    .map((s) => s[0])
    .slice(0, 2)
...
const avatarStyle = computed(() => {
  ...
  return { backgroundColor: 'var(--primary-color, #00e5ff)' }; // Fallback color
});
...
const loadData = async () => {
  ...
  try {
    ...
    if (profileData) {
      ...
      // Fallback se não existir perfil criado ainda
...
// 1. Atualizar Nome (Tabela Profiles)
const updateName = async () => {
  ...
  try {
    ...
    // Atualiza também o metadata do Auth para garantir sincronia
...
// 3. Segurança (Email/Senha)
const updateEmail = async () => {
  if (!security.value.email) return;
  loading.value = true;
  try {
    const { error } = await supabase.auth.updateUser({ email: security.value.email });
    if (error) throw error;
...
const updatePassword = async () => {
  ...
  loading.value = true;
  try {
    const { error } = await supabase.auth.updateUser({ password: security.value.password });
    if (error) throw error;
...
/* SIDEBAR STYLES */
.sidebar {
  background: rgba(255, 255, 255, 0.05); /* Levemente mais escuro que o vidro principal */
...
.nav-menu {
  ...
  flex-direction: column;
  gap: 0.5rem;
  flex: 1; /* Empurra o logout para baixo */
}
