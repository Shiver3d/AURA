...
<template>
  <div class="search-root container">
    <h2>Produtos</h2>
    <div class="grid">
      <div v-for="p in products" :key="p.id" class="product card glass" @click="open(p)">
        <img :src="p.image" alt="" />
        <h4>{{ p.name }}</h4>
        <div class="tags">
    ...
    <div v-if="selected" class="modal-overlay" @click.self="selected = null">
      <div class="modal glass">
        ...
        <section class="reviews">
          <h4>Avaliações</h4>
          <ul>
            <li v-for="r in selected.reviews" :key="r.id">{{ r.author }} — {{ r.text }}</li>
          </ul>
        </section>
      </div>
    </div>
...
const products = ref([
  { id:1, name:'La Roche-Posay Effaclar', brand:'La Roche-Posay', sustainable:true, refill:false, image:'/src/assets/sample1.jpg', description:'Cuidado para pele oleosa.', reviews:[{id:1,author:'Ana',text:'Excelente!'}] },
  { id:2, name:'Vichy Mineral 89', brand:'Vichy', sustainable:false, refill:true, image:'/src/assets/sample2.jpg', description:'Hidratante leve.', reviews:[{id:2,author:'Carlos',text:'Rápido efeito.'}] }
])
...
<style scoped>
.grid { display:grid; grid-template-columns: repeat(auto-fit,minmax(200px,1fr)); gap:18px; margin-top:16px; }
.product { padding:12px; cursor:pointer; }
...
.tag { background:rgba(0,0,0,0.04); padding:4px 8px; border-radius:999px; font-size:12px; }
.tag.green { background: linear-gradient(90deg,#e6fff7,#d0fff0); }
