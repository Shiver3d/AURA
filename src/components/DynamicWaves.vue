...
function drawWaves(el) {
  ...
  layers.value.forEach((layer, idx) => {
    ...
    for (let i=0;i<=samples;i++){
      ...
      const n = valueNoise1D(i * layer.freq * 60 + idx * 47 + time * layer.speed * 1 * speedMultiplier.value)
    ...
    const grad = ctx.createLinearGradient(0, height*0.25, width, height*0.75)
    if (layer.colors && layer.colors.length >= 2) {
      grad.addColorStop(0, layer.colors[0].replace(/ALPHA/g, String(layer.alpha || 0.12)))
      grad.addColorStop(1, layer.colors[1].replace(/ALPHA/g, String(layer.alpha || 0.12)))
    } else if (idx === 2) {
      ...
      grad.addColorStop(1, 'rgba(111,214,255,' + ((layer.alpha || 0.12) * 0.9) + ')')
    }

    ctx.strokeStyle = grad
    ctx.lineWidth = layer.lw || 1
    ctx.lineCap = 'square'
...
onMounted(()=>{
  ...
  const onResize = () => {
    ...
    requestAnimationFrame(() => { resizeScheduled = false; resizeCanvas(el) })
  }
...
onBeforeUnmount(()=>{
  const el = canvas.value
  if (el && el.__dw_onResize) window.removeEventListener('resize', el.__dw_onResize)
